...
5. **Maintain strict modularity via dedicated subsystem files**: When introducing new architectural domains (e.g., ORM & Relational DB, Web Client-Server), implement fresh design system factories (`web_client_server_design_system.js`) and scene modules (`web_client_server_scenes.js`) rather than bloating existing files. This prevents accidental regressions in previously stabilized stages.
6. **Explicit Dual-Endpoint Code Linking**: Visualizing network operations requires presenting both Client-Side (`fetch('/api/v1/orders/1042')`) and Server-Side (`@app.get('/api/v1/orders/{order_id}') -> def handler(...)`) code simultaneously. Illuminating the Python decorator when the request capsule arrives establishes the crucial mental model that decorators are routing switches directing incoming network streams into specific function arguments.
7. **Bidirectional Network Transit Pacing**: When coordinating round-trip animations (Client -> Server -> Database -> Server -> Client), stagger each leg with deliberate transit (3.0s–3.5s) and 1.8s–2.2s dwell at key inspection nodes (schema validation, DB query, 200/201/204 response assembly) to allow the learner's eye to follow the cause-and-effect cascade.
8. **Comprehensive Rectangle Bounds & Word-Wrapping Protection**: In high-DPI viewports, monospace text without explicit bounds or `wordWrap: { width, useAdvancedWrap: true }` easily spills across container edges when code lines, route paths, or JSON payloads are rendered. Always size container chassis generously (e.g. 410px station width, 260px packet width) and enforce wrapping across all label, badge, and code text elements.
9. **Micro-Step Request/Response Assembly & Wire Serialization**: Visualizing JSON creation requires decomposing the build into discrete visual sub-steps (Target URL -> Headers -> Key 1 -> Key 2 -> JSON.stringify sealing) with 1.8s–2.0s per step, followed by deliberate 3.5s wire transit. This connects the mental model of client-side heap state transforming into raw serialized byte tokens traveling across physical network wire.
